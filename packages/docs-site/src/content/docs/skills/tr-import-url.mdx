...
For WordPress sources, use [tr-migrate-wp](../tr-migrate-wp/) instead.
...
- Gallery blocks → the AI agent converts to CSS grid with `<img>` tags

### Wix

- Check what each public page returns; JavaScript and access restrictions can limit direct fetching
- the AI agent will ask you for an HTML export or take screenshots as reference
- Screenshots are layout references, not a substitute for text and original media
...
See the separate guides for [Squarespace](../../guides/migrate-from-squarespace/),
[Wix](../../guides/migrate-from-wix/) and [Webflow](../../guides/migrate-from-webflow/)
for migration scope, exclusions and review steps.
...
- Images (transferred through Typeroll’s media storage)
- Text content and structure

<Aside type="caution">
  Forms from Squarespace/Wix are vendor-locked and won't work after import. the
  AI agent will create new Typeroll forms in their place.
</Aside>

## Drafts first, always

All imported pages start as `status: "draft"`. The AI agent shows you a preview and asks for approval before publishing. This gives you a chance to review the content before it goes live.
