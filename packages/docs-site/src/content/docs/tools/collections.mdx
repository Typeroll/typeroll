...
Returns all items in a collection. The AI agent uses this to regenerate listing HTML when new items are added.
...
- **`item_template_blocks`** — `Block[]` tree using `template/item_*` family bindings (preferred for new collections). Bound to the current item's fields via `{{item.title}}`, `{{item.body}}`, etc. — the AI agent doesn't have to type these tokens by hand; the `template/item_title`, `template/item_body`, `template/item_image` blocks read from context.
...
Use [block instance tools](../blocks/) with `target: { kind: "item_template", id: "<collection_name>" }` to edit the layout:
