...
it('freezes the selected branch including inherited blocks, templates, partials and typed pages', async () => {
  ...
  await store.setDoc(paths.blockType('org', 'site', 'custom-heading', 'main'), { name: 'custom-heading', schema: [{ name: 'text', type: 'text' }], template: '<h1>Main template</h1>' });
  await store.setDoc(paths.blockType('org', 'site', 'custom-heading', 'design'), { name: 'custom-heading', schema: [{ name: 'text', type: 'text' }], template: '<h2>Branch template</h2>' });
  await store.setDoc(paths.pageTemplate('org', 'site', 'article', 'main'), { name: 'article', status: 'active' });
  await store.setDoc(paths.partial('org', 'site', 'footer', 'main'), { content_mode: 'html', html_content: 'Inherited footer', status: 'published' });
  ...
  await store.setDoc(paths.page('org', 'site', 'removed', 'main'), { title: 'Still on main', content_mode: 'html', status: 'published' });
