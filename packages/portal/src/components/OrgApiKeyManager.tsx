// Org-scoped API key manager — paired with /app/settings/api-keys.astro.
// Identical shape to ApiKeyManager but targets /api/orgs/api-keys instead
// of /api/sites/{siteId}/api-keys. Surfaces "use this for the hosted MCP
// connector" copy in the show-once reveal modal.
...
export default function OrgApiKeyManager({ initialKeys }: Props) {
  ...
  async function revoke(prefix: string, label: string) {
    ...
    try {
      const res = await fetch(`/api/orgs/api-keys/${prefix}`, { method: 'DELETE' });
  ...
  const origin = typeof window !== 'undefined' ? window.location.origin : 'https://app.typeroll.com';
  const claudeConnectorUrl = `${origin}/api/mcp`;
  const stdioSnippet = newToken
    ? JSON.stringify(
        {
          mcpServers: {
            typeroll: {
    ...
    <div className="stack">
      <div className="card stack">
        <div className="row" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h2 style={{ fontSize: '1.125rem', margin: 0 }}>Org-scoped API keys</h2>
            <p className="muted text-sm" style={{ margin: '0.25rem 0 0' }}>
              Span every site in your org and any site shared into your org. Use one of these for the hosted Claude connector.
            </p>
          </div>
          {!creating && (
        ...
        {keys.length === 0 && !creating && (
          <p className="muted text-sm" style={{ margin: 0 }}>
            No org-scoped keys yet. Create one to wire up the hosted Claude connector.
          </p>
        )}
      ...
      {newToken && newKeyMeta && (
        <div className="modal-backdrop" onClick={() => setNewToken(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <h4 style={{ marginBottom: '0.25rem' }}>Add to Claude Desktop / claude.ai</h4>
            <p className="text-sm muted" style={{ margin: 0 }}>
              In Claude, open Settings → Connectors → Add custom connector and paste this URL:
            </p>
            <pre className="modal__snippet"><code>{claudeConnectorUrl}</code></pre>
            <p className="text-sm muted" style={{ margin: 0 }}>
              You'll be redirected to a consent screen — paste the key above into the form.
            </p>

            <h4 style={{ marginBottom: '0.25rem' }}>Or use stdio with Claude Code</h4>
            <p className="text-sm muted" style={{ margin: 0 }}>
              Add this to your <code>.claude.json</code>:
            </p>
            <pre className="modal__snippet"><code>{stdioSnippet}</code></pre>
