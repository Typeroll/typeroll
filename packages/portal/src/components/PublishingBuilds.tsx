...
export default function PublishingBuilds({ refreshAfterCloudflareReturn = false }: { refreshAfterCloudflareReturn?: boolean }) {
  ...
  return <PublishingCard id="publishing-builds" title="Builds"
    ...
    {!!settings?.active_jobs.length && <section className="publishing-builds__activity" aria-label="Active builds"><h3>Active builds</h3><ul>{settings.active_jobs.map(job => <li key={job.key}>
      <div><strong>{job.site_name}</strong><span>{job.version_id} · {job.provider === 'github' ? 'GitHub Actions' : 'Cloudflare'} · {job.status === 'running' ? 'Building' : 'Queued'}</span>{job.media_preparation && job.media_preparation.completed < job.media_preparation.total && <span role="status">Preparing media: {job.media_preparation.completed} of {job.media_preparation.total} files ready. Continues automatically; you can close this page.</span>}</div>
