...
export function qualificationSource(id: string) {
  return { 'qualification.mjs': `import fs from 'node:fs/promises'; import path from 'node:path'; import os from 'node:os';
const status=await fs.readFile('/proc/self/status','utf8');
if(os.userInfo().uid!==1000||process.getuid()!==1000||!/NoNewPrivs:\\s+1/.test(status)||!/CapEff:\\s+0+\\s/.test(status)||process.env.TYPEROLL_RUNNER_TOKEN||process.env.CLOUDFLARE_API_TOKEN)throw Error('isolation_failed');
