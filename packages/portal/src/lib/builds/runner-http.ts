...
import { authorizeEngine, readEngineConfiguration, buildInputPath, engineConfigurationPath, type BuildInput, type EngineConfiguration } from './state';
...
export async function runnerRequest(request: Request, org: string, action: string) {
  try {
    ...
    if (action === 'claim') {
      const engine = await authorizeEngine(org, token, String(input.revision));
      if (!rateLimit(`build-claim:${org}`, 30, 60000).allowed) return privateJson({ error: 'Too many build claims.' }, 429);
      const claim = await queue.claim(org, engine.revision, Number(input.protocol));
    ...
    const engine = await readEngineConfiguration(org);
    ...
    if (metadata.kind === 'qualification') {
      const activated = await store.compareAndUpdateDoc<EngineConfiguration>(engineConfigurationPath(org), value => value.revision === engine.revision && value.status === 'qualifying', { status: 'ready' });
      if (activated) await store.updateDoc(enginePath(org), { revision: randomUUID(), state: 'ready', enabled: true, checked_at: new Date().toISOString(),
