...
export const engineConfigurationPath = (org: string) => `organizations/${org}/publishing_private/build_engine`;
export const buildInputPath = (org: string, key: string) => `organizations/${org}/build_inputs/${key}`;
export interface BuildInput { source_key: string; kind: 'qualification' | 'publication'; dispatch_id?: string; dispatch_uncertain?: boolean; dispatch_started_at?: number; dispatch_attempt?: number; dispatch_lease_until?: number; storage_account_id: string }
export async function readEngineConfiguration(org: string) { return getStore().getDoc<EngineConfiguration>(engineConfigurationPath(org)); }
