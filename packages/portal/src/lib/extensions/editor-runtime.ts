...
export async function buildExtensionEditorRuntimeScript(
  ...
  const snapshot = await buildExtensionRuntimeSnapshot(orgId, siteId);
...
function nestedHost(payload){
  ...
  var navigation=nav();var api={fetch:function(){return Promise.reject(new Error("Extension API calls are disabled in editor preview"));}};var forms={has:function(id){return payload.form_binding_ids.includes(id);},list:function(){return payload.form_binding_ids.slice();},submit:function(){return Promise.reject(new Error("Form submissions are disabled in preview"));}};
