...
function labelFor(item: Page): string {
  const d: Record<string, unknown> = { ...item.fields, ...item };
...
export function analyzeCompleteness(
  coll: Pick<ContentType, 'name' | 'fields'>,
  items: Page[],
  ...
  const analysed: PageCompleteness[] = [];
  ...
  for (const item of items) {
    ...
    const stale: PageCompleteness['stale'] = [];
  ...
  return {
    content_type: coll.name,
    total_pages: items.length,
    incomplete_pages: analysed.filter((i) => i.missing.some((m) => m.required)).length,
