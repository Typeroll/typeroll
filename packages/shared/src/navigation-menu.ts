...
[data-block="navigation_menu"] .block-menu-scroll { position:absolute; inset:var(--menu-header-bottom,0px) 0 0; overflow:auto; overscroll-behavior:contain; background:var(--menu-background,#fff); }
...
export const navigationLinks: BlockType = {
  ...
  template:`<ul data-block="navigation_links" data-font="{{font}}" style="--direction:{{direction}};--link_color:{{color}};--font_weight:{{font_weight}}">{{{navigation_links_html}}}</ul>`,
...
[data-block="navigation_links"] > li { list-style:none;margin:0;padding:0; }
[data-block="navigation_links"] > li::marker { content:''; }
[data-block="navigation_links"] > li > a { display:flex;align-items:center;gap:var(--icon_gap_px,12px);color:var(--link_color,inherit);font-size:var(--font_size_px,inherit);line-height:var(--line_height,1.4);font-weight:var(--font_weight,400);padding-block:var(--padding_y_px,8px);text-decoration:none;overflow-wrap:anywhere; }
