...
import { MEDIA_RECIPE_VERSION } from './media-recipe.mjs';
...
export function mediaReceiptKey(manifest, entry) {
  ...
  return entry.public_key + `.prepared-${MEDIA_RECIPE_VERSION}.` + createHash('sha256').update(JSON.stringify(scope)).digest('hex') + '.json';
}
